...
export default function BulkExamples() {
  ...
  useEffect(() => {
    ...
    adminCategoryWordCounts().then((r) => {
      ...
      const RELATED_CATS = ['ทั่วไป', 'ชีวิตประจำวัน', 'ธุรกิจ', 'กฎหมาย', 'สำนวน']
        ...
        {activeTab === 'คำเกี่ยวข้อง' && (
          <>
            ...
            <div className="border-t border-gray-100 pt-3">
              ...
              <p className="text-xs text-gray-400 mb-3">เขียนทับ related_words ของทุกคำในหมวดที่เลือก (เฉพาะ ทั่วไป/ชีวิตประจำวัน/ธุรกิจ/กฎหมาย/สำนวน)</p>
              <div className="flex gap-2 mb-3">
                <select
                ...
                >
                  {categoryWordCounts ? (
                    categoryWordCounts.categories
                      .filter((c) => ['ทั่วไป', 'ชีวิตประจำวัน', 'ธุรกิจ', 'กฎหมาย', 'สำนวน'].includes(c.name))
                      .map((c) => (
                        <option key={c.name} value={c.name}>{c.name} ({c.count})</option>
                      ))
                  ) : (
