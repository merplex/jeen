...
def _translate_lines_with_vocab(lines: list, all_words: list) -> str:
    ...
    is_conversation = len(blocks) >= 2

    if is_conversation:
        conv_instruction = (
            "ข้อความนี้มีหลายบรรทัด วิเคราะห์ว่าเป็นบทสนทนาระหว่างคนสองคนหรือมากกว่าไหม\n"
            "ถ้าใช่ → ทุกบรรทัดต้องขึ้นต้นด้วย A: หรือ B: (สลับกันตามลำดับที่พูด) "
            "ถ้ารู้ชื่อจากภาพให้ใช้ชื่อแทน A/B\n"
            "ถ้าไม่ใช่บทสนทนา → แปลตรงๆ ไม่ต้องใส่ A: B:\n"
        )
    else:
        conv_instruction = ""
