...
def batch_generate_examples(words: list[dict]) -> dict[int, list[dict]]:
    ...
    try:
        ...
        for w in words:
            ...
            meaning_lines = meaning_lines[:3]  # cap ที่ 3 บรรทัดแรก ป้องกัน output truncation
...
def generate_related_words(chinese: str, pinyin: str, thai_meaning: str) -> dict:
    ...
    if is_idiom:
        prompt = (
            ...
            "ต้องใช้อักษรจีนตัวย่อ (简体) เท่านั้น ห้ามใช้ตัวเต็ม (繁體) "
            "พยายามหาคำให้ได้เสมอ ยอมรับคำที่ใกล้เคียงพอสมควรได้ "
            "ให้ตอบ array ว่าง [] เฉพาะกรณีที่ไม่มีสำนวนใดในภาษาจีนที่เข้าข่ายได้เลยจริงๆ (หายากมาก)\n"
        ...
        prompt = (
            ...
            '{"similar": [คำที่มีความหมายคล้ายกัน 2 คำ แต่ละคำต้องมี 2 อักษรจีนเท่านั้น], '
